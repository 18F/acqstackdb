    ...
    if created:
        ...
        if statuses_length > 1 and instance.is_before_id:
            try:
                ...
                current_before = AwardStatus.objects.get(track=instance.track, is_before_id=instance.is_before_id, id__lt=instance.id)
            ...
            next_status = AwardStatus.objects.get(track=instance.track, id=instance.is_before_id)
            # Find all objects with that order or higher and increment all filtered objects by 1
            later_statuses = AwardStatus.objects.filter(track=instance.track, ordering__gte=next_status.ordering).update(ordering=F('ordering') + 1)
        elif statuses_length > 1:
            # This is now the last award status in the process
            last_status = AwardStatus.objects.filter(track=instance.track, ordering__gte=0).order_by('ordering').last()
...
def decrement_ordering(instance, **kwargs):
    current_before = AwardStatus.objects.get(track=instance.track, is_before_id=instance.id)
